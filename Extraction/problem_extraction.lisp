...
(defun parseProblem (in)
  (cond
   ...
   ((eq :objects (caar in)) (progn (print "ob") (setq objList (reverse (objectConvertList (cdar in) ()))) (parseProblem (cdr in))))
...
(defun convert (obj)
	(if (eq nil obj)
		...
		(defun convertAlt (obj)
			(if (eq nil obj)
				""
			(stringconvert (string obj))))

;Convert object list into string
(defun objectConvertList (objL st)
		(let* (	(s (convertAlt (car objL)))
						(nst (cons s st)))
		(if (cdr objL)
			(objectConvertList (cdr objL) nst)
			nst)))
